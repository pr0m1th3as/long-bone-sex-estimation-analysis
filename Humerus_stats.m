...
HumerusData = {'Variables', 'Males', 'Females', 'M_Lo', 'M_Hi', 'F_Lo', 'F_Hi'};
M_bs = []; M_bs_out = {};
F_bs = []; F_bs_out = {};
Pval_CI95 = [];
...
for i = 6:66
  ## Males both sides
  Msample = Humerus(Humerus(:,4) == 1, [1, 2, 3, i]);
  ...
  ## Females both sides
  Fsample = Humerus(Humerus(:,4) == 2, [1, 2, 3, i]);
  ...
  ## Aggregate Humerus data sample without outliers only from variables
  ## that exhibit sexual dimorphism
  if (pval < 0.05)
    HumerusData = [HumerusData; Header(i), {M}, {F}, ...
...
HumerusOutlierIDs = [{'Variables', 'Males', 'Females'}; ...
                     Header(6:66)', M_bs_out(:), F_bs_out(:)];

#HumerusStats = table (M_bs(:,[1:2]), F_bs(:,[1,2]), Pval_CI95(:,1) < 0.05, ...
#                      Pval_CI95(:,1), Pval_CI95(:,[2:3]), Pval_CI95(:,4), ...
#                      size (Humerus, 1) - sum ([M_bs(:,3), F_bs(:,3)], 2), ...
...
Midx = [];
Fidx = [];
for i = 2:size (HumerusData, 1)
  Midx = [Midx; HumerusData{i,2}(:,[1:3])];
  Fidx = [Fidx; HumerusData{i,3}(:,[1:3])];
endfor
...
Xh = [Midx(:,4:end); Fidx(:,4:end)];
Yh = [ones(size(Midx(:,1))); 2*ones(size(Fidx(:,1)))];
H_mu = mean (Xh, 'omitnan');
Hstd = std (Xh, 'omitnan');
Zh = (Xh - H_mu) ./ Hstd;

## Save variables and clean up workspace
clear -x HumerusData HumerusOutlierIDs HumerusStats Xh Yh Zh H_mu Hstd
save ('-binary', 'Humerus-Data.mat', 'HumerusData', 'HumerusOutlierIDs', ...
      'Xh', 'Yh', 'Zh', 'H_mu', 'Hstd');
