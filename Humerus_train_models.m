...
fprintf ("Training Humerus models: %d samples (%d males, %d females)\n\n", ...
         length (Yh), sum (Yh == 1), sum (Yh == 2));

## Save classification models into a cell array
models = {};

## Get scores from multivariate classification using all measurements
mdl = fitcdiscr (Zh, Yh, "Gamma", 0.3, "Prior", "uniform");
mdl = compact (mdl);
[out, Scores] = predict (mdl, Zh);
acc = sum (out == Yh) / sum (TF);
models = [models; {mdl}];
fprintf ("Multivariable Linear Discriminant Classification yields %0.2f%% accuracy.\n", acc * 100);

mdl = fitcknn (Zh, Yh, "NumNeighbors", 10);
[out, score] = predict (mdl, Zh);
acc = sum (out == Yh) / sum (TF);
models = [models; {mdl}];
...
for i = 1:size (Vidx, 1)
  Z = Zh(:,i);
  mdl = fitcdiscr (Z, Yh, "Gamma", 0.3, "Prior", "uniform");
  mdl = compact (mdl);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  LDA = [LDA, {[out, score, Yh]}];
  Scores = [Scores, score];

  mdl = fitcknn (Z, Yh, "NumNeighbors", 20);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  KNN = [KNN, {[out, score, Yh]}];
  Scores = [Scores, score];
endfor

## Train a FCNN model on aggregated scores
mdl = fitcnet (Scores, Yh, 'LayerSizes', [80,40,20]);
mdl = compact (mdl);
...
fprintf ("Total accuracy for Humerus model: %0.2f%%\n\n", fms);
