...
Zf = Zf(:,Vidx(:,1));
TF = ! logical (sum (isnan (Zf), 2));
Zf = Zf(TF,:);
Yf = Yf(TF);

## Print sample information
fprintf ("Training Femur models: %d samples (%d males, %d females)\n\n", ...
         length (Yf), sum (Yf == 1), sum (Yf == 2));

## Save classification models into a cell array
models = {};

## Get scores from multivariate classification using all measurements
mdl = fitcdiscr (Zf, Yf, "Gamma", 0.3, "Prior", "uniform");
mdl = compact (mdl);
[out, Scores] = predict (mdl, Zf);
acc = sum (out == Yf) / sum (TF);
models = [models; {mdl}];
fprintf ("Multivariable Linear Discriminant Classification yields %0.2f%% accuracy.\n", acc * 100);

mdl = fitcknn (Zf, Yf, "NumNeighbors", 10);
[out, score] = predict (mdl, Zf);
acc = sum (out == Yf) / sum (TF);
models = [models; {mdl}];
...
for i = 1:size (Vidx, 1)
  Z = Zf(:,i);
  mdl = fitcdiscr (Z, Yf, "Gamma", 0.3, "Prior", "uniform");
  mdl = compact (mdl);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  LDA = [LDA, {[out, score, Yf]}];
  Scores = [Scores, score];

  mdl = fitcknn (Z, Yf, "NumNeighbors", 20);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  KNN = [KNN, {[out, score, Yf]}];
  Scores = [Scores, score];
endfor

## Train a FCNN model on aggregated scores
mdl = fitcnet (Scores, Yf, 'LayerSizes', [80,40,20]);
mdl = compact (mdl);
[out, s] = predict (mdl, Scores);
models = [models; {mdl}];
fprintf ("\nFCNN Classification on Scores yields %0.2f%% accuracy.\n\n", ...
         (sum (out == Yf) / sum (TF)) * 100);
...
M = [Yf, m_1, f_1, m09, f09, m08, f08, m07, f07, m06, f06, m05, f05];
...
fprintf ("Total accuracy for Femur model: %0.2f%%\n\n", fms);
