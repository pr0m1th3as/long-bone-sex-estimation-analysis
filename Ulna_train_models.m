...
Zu = Zu(:,Vidx(:,4));
TF = ! logical (sum (isnan (Zu), 2));
Zu = Zu(TF,:);
Yu = Yu(TF);

## Print sample information
fprintf ("Training Ulna models: %d samples (%d males, %d females)\n\n", ...
         length (Yu), sum (Yu == 1), sum (Yu == 2));

## Save classification models into a cell array
models = {};

## Get scores from multivariate classification using all measurements
mdl = fitcdiscr (Zu, Yu, "Gamma", 0.3, "Prior", "uniform");
mdl = compact (mdl);
[out, Scores] = predict (mdl, Zu);
acc = sum (out == Yu) / sum (TF);
models = [models; {mdl}];
fprintf ("Multivariable Linear Discriminant Classification yields %0.2f%% accuracy.\n", acc * 100);

mdl = fitcknn (Zu, Yu, "NumNeighbors", 10);
[out, score] = predict (mdl, Zu);
acc = sum (out == Yu) / sum (TF);
models = [models; {mdl}];
...
for i = 1:size (Vidx, 1)
  Z = Zu(:,i);
  mdl = fitcdiscr (Z, Yu, "Gamma", 0.3, "Prior", "uniform");
  mdl = compact (mdl);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  LDA = [LDA, {[out, score, Yu]}];
  Scores = [Scores, score];

  mdl = fitcknn (Z, Yu, "NumNeighbors", 20);
  [out, score] = predict (mdl, Z);
  models = [models; {mdl}];
  KNN = [KNN, {[out, score, Yu]}];
  Scores = [Scores, score];
endfor

## Train a FCNN model on aggregated scores
mdl = fitcnet (Scores, Yu, 'LayerSizes', [80,40,20]);
mdl = compact (mdl);
[out, s] = predict (mdl, Scores);
models = [models; {mdl}];
fprintf ("\nFCNN Classification on Scores yields %0.2f%% accuracy.\n\n", ...
         (sum (out == Yu) / sum (TF)) * 100);
...
M = [Yu, m_1, f_1, m09, f09, m08, f08, m07, f07, m06, f06, m05, f05];
