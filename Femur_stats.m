...
FemurData = {'Variables', 'Males', 'Females', 'M_Lo', 'M_Hi', 'F_Lo', 'F_Hi'};
M_bs = []; M_bs_out = {};
F_bs = []; F_bs_out = {};
Pval_CI95 = [];
...
for i = 6:66
  ## Males both sides
  Msample = Femur(Femur(:,4) == 1, [1, 2, 3, i]);
  ...
  ## Females both sides
  Fsample = Femur(Femur(:,4) == 2, [1, 2, 3, i]);
  ...
  ## Aggregate Femur data sample without outliers only from variables
  ## that exhibit sexual dimorphism
  if (pval < 0.05)
    FemurData = [FemurData; Header(i), {M}, {F}, ...
...
FemurOutlierIDs = [{'Variables', 'Males', 'Females'}; ...
                   Header(6:66)', M_bs_out(:), F_bs_out(:)];

#FemurStats = table (M_bs(:,[1:2]), F_bs(:,[1,2]), Pval_CI95(:,1) < 0.05, ...
#                    Pval_CI95(:,1), Pval_CI95(:,[2:3]), Pval_CI95(:,4), ...
#                    size (Femur, 1) - sum ([M_bs(:,3), F_bs(:,3)], 2), ...
...
Midx = [];
Fidx = [];
for i = 2:size (FemurData, 1)
  Midx = [Midx; FemurData{i,2}(:,[1:3])];
  Fidx = [Fidx; FemurData{i,3}(:,[1:3])];
endfor
...
Xf = [Midx(:,4:end); Fidx(:,4:end)];
Yf = [ones(size(Midx(:,1))); 2*ones(size(Fidx(:,1)))];
F_mu = mean (Xf, 'omitnan');
Fstd = std (Xf, 'omitnan');
Zf = (Xf - F_mu) ./ Fstd;

## Save variables and clean up workspace
clear -x FemurData FemurOutlierIDs FemurStats Xf Yf Zf F_mu Fstd
save ('-binary', 'Femur-Data.mat', 'FemurData', 'FemurOutlierIDs', ...
      'Xf', 'Yf', 'Zf', 'F_mu', 'Fstd');
