...
TibiaData = {'Variables', 'Males', 'Females', 'M_Lo', 'M_Hi', 'F_Lo', 'F_Hi'};
M_bs = []; M_bs_out = {};
F_bs = []; F_bs_out = {};
Pval_CI95 = [];
...
for i = 6:66
  ## Males both sides
  Msample = Tibia(Tibia(:,4) == 1, [1, 2, 3, i]);
  ...
  ## Females both sides
  Fsample = Tibia(Tibia(:,4) == 2, [1, 2, 3, i]);
  ...
  ## Aggregate Tibia data sample without outliers only from variables
  ## that exhibit sexual dimorphism
  if (pval < 0.05)
    TibiaData = [TibiaData; Header(i), {M}, {F}, ...
...
TibiaOutlierIDs = [{'Variables', 'Males', 'Females'}; ...
                   Header(6:66)', M_bs_out(:), F_bs_out(:)];

#TibiaStats = table (M_bs(:,[1:2]), F_bs(:,[1,2]), Pval_CI95(:,1) < 0.05, ...
#                    Pval_CI95(:,1), Pval_CI95(:,[2:3]), Pval_CI95(:,4), ...
#                    size (Tibia, 1) - sum ([M_bs(:,3), F_bs(:,3)], 2), ...
...
Midx = [];
Fidx = [];
for i = 2:size (TibiaData, 1)
  Midx = [Midx; TibiaData{i,2}(:,[1:3])];
  Fidx = [Fidx; TibiaData{i,3}(:,[1:3])];
endfor
...
Xt = [Midx(:,4:end); Fidx(:,4:end)];
Yt = [ones(size(Midx(:,1))); 2*ones(size(Fidx(:,1)))];
T_mu = mean (Xt, 'omitnan');
Tstd = std (Xt, 'omitnan');
Zt = (Xt - T_mu) ./ Tstd;

## Save variables and clean up workspace
clear -x TibiaData TibiaOutlierIDs TibiaStats Xt Yt Zt T_mu Tstd
save ('-binary', 'Tibia-Data.mat', 'TibiaData', 'TibiaOutlierIDs', ...
      'Xt', 'Yt', 'Zt', 'T_mu', 'Tstd');
